...
_Remember?_ The `<head>` element is a container for processing information and document metadata. The `<head>` contains high-level information about the site, and always comes first within the root element. In this sense, we can consider the `<head>` to contain the thoughts of the page.
